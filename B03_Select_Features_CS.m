...
aa=load([root_dataRead1, '\Rank_CS_NSelect',num2str(NumSelect),'_norm',num2str(idx_norm),'.mat']);
...
file_w=[root_dataSave, '\mat_norm',num2str(idx_norm),'\Loop',num2str(num),'\B_stat_abs_NumFS',num2str(num_fs),'.xlsx'];
...
%%
    for num_fs = 1:NumSelect
        ...
        TrainSelect = TrainData(:, [1; idx+1]);
        TestSelect = TestData(:, [1; idx+1]);
file_w=[root_dataSave, '\mat_norm',num2str(idx_norm),'\Loop',num2str(num),'\B_stat_cnt_NumFS',num2str(num_fs),'.xlsx'];
        writetable(TrainSelect, file_w, 'Sheet','TrainSelect')
        writetable(TestSelect, file_w, 'Sheet','TestSelect')
    end
%%
    for num_fs = 1:NumSelect
        ...
        TrainSelect = TrainData(:, [1; idx+1]);
        TestSelect = TestData(:, [1; idx+1]);
file_w=[root_dataSave, '\mat_norm',num2str(idx_norm),'\Loop',num2str(num),'\RE_stat_cnt_NumFS',num2str(num_fs),'.xlsx'];
        writetable(TrainSelect, file_w, 'Sheet','TrainSelect')
        writetable(TestSelect, file_w, 'Sheet','TestSelect')
    end
